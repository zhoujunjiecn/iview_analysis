...
//                    const width = this.bodyHeight === 0 ? this.tableWidth : this.tableWidth - this.scrollBarWidth;
        ...
        methods: {
            ...
            /*  
                ...
                * @columns 每列可以设置 children， 设置子集
                * 
                * > 递归 给每项设置 __id 属性 (得到随机的6位字符串)
                * 
                * @ 返回列数据
            */
            ...
            /*
                ...
                * getAllColumns 铺平多层嵌套 带有children属性
                ...
                1. 深拷贝一份 被铺平列的数据
                2. 定义三个数组  left（存放被固定在左侧的列）、center（没有固定的列）、right（被固定在右侧的列）
                3. 遍历
                    3.1 给每项设置属性 _index (保存值的索引，0开始自增)
                    3.2 给每项设置属性 _columnKey (保存列的索引，1开始自增)
                    3.3 给每项设置属性 _width           = column.width ? column.width : ''
                    3.4 给每项设置属性 _sortType        = column.sortType ? column.sortType : 'normal'
                    3.5 给每项设置属性 _filterVisible   = false
                    3.6 给每项设置属性 _isFiltered      = column.filteredValue ? true : false
                    3.7 给每项设置属性 _filterChecked   = column.filteredValue ? column.filteredValue : []
                    3.8 给每项设置属性 _filterMultiple  = column.filterMultiple ? column.filterMultiple : true;

                4. 返回合并了 left、center、right 的数组
            */
            makeColumns (cols) {
